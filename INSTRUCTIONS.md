...
2. **Proceso Obligatorio de Respaldo y Despliegue (Flujo de Trabajo):**
   - Siempre que se realice un respaldo, entrega o actualización, se debe seguir estrictamente este orden de ejecución:
     ...
     3. **Respaldo local en la carpeta `.backup`:** Comprimir los archivos del proyecto (excluyendo carpetas pesadas/de sistema como `.git`, `.backup`, `.kilo` y `node_modules`) en un archivo ZIP nombrado bajo la convención: `Respaldo_Rack_Designer_YYYYMMDD_HHMM.zip`.
     4. **Respaldo en Git:** Realizar la confirmación de cambios (`git commit`) y enviarlos a la rama activa del repositorio remoto (`git push`).
